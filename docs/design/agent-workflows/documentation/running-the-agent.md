...
- `services`. Runs uvicorn on port `8080` inside the container
  ...
  `http://runner:8765` and `AGENTA_AGENT_MCPS_ENABLED` to `true` by default (lines 564 to
...
- `AGENTA_AGENT_MCPS_ENABLED`. Gates user MCP server resolution (http-only; stdio stays
  refused by design). Default `true`; set `false` to disable. The SSRF guard covers scheme
  and host literals; DNS-rebinding and redirect hardening is tracked in #4911.
